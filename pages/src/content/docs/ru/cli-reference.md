...
| `--no-summary` | — | `false` | Пропускает вызовы LLM `CHANGE_SUMMARY_TASK`, `IMPACT_ANALYSIS_TASK` и `FLOW_DIAGRAM_TASK` после завершения ревью. |
...
| `project_summary` | Необязательно. Сводка на уровне проекта для режима сканирования (markdown). Пусто для `ocr review`. |
| `change_summary` | Необязательно. Сводка изменений в режиме ревью (markdown): намерение, затронутые модули, статистика, ключевые решения. Создаётся вызовом LLM `CHANGE_SUMMARY_TASK` после ревью. Опускается при `--no-summary` или сбое задачи. |
| `impact_analysis` | Необязательно. Анализ бизнес-влияния в режиме ревью (markdown): затронутая функциональность, изменения контрактов, оценка рисков, области, подверженные регрессии. Создаётся вызовом LLM `IMPACT_ANALYSIS_TASK` после ревью. |
| `flow_diagram` | Необязательно. Строка с диаграммой Mermaid в режиме ревью, показывающая влияние на цепочку вызовов/поток данных и бизнес-процесс до/после. Создаётся вызовом LLM `FLOW_DIAGRAM_TASK` после ревью. |
